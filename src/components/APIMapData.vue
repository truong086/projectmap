<template>
    ...
    <!-- <select v-model="selectedTransport" @change="getDirections">
      ...
      <option value="driving">Máy bay2</option>
      <option value="driving-car">Xe máy</option>
    </select> -->
 ...
 <GMapMap
      ...
      >
      <!-- Công cụ vẽ đa giác -->
      <!-- <GMapDrawingManager ref="drawingManager" :options="drawingOptions" /> -->
            ...
            <!-- <GMapOverlay
              ...
              >
                <div class="custom-marker">
                  <img :src="imageStatus.status3" alt="Marker" />
                </div>
              </GMapOverlay> -->
             <GMapInfoWindow
                    v-if="selectedMarker === index && showDistanceList"
...
// Hàm lấy tọa độ của đa giác
// const onPolygonComplete = (polygon) => {
//       console.log("Đa giác hoàn thành:", polygon);
...
//       console.log("Tọa độ của đa giác:", coordinates);
//     };
  ...
  const getCurrentLocation = () => {
    if (navigator.geolocation) {
       ...
       * navigator.geolocation.getCurrentPosition(
        ...
        {
          enableHighAccuracy: true,
          timeout: 10000,   // Tăng thời gian timeout lên 10 giây
          maximumAge: 0     // Luôn cập nhật vị trí mới nhất
        }// Bật độ chính xác cao
      );
       */
    }
  };
...
const midPoint = computed(() => {
  ...
  onMounted(() => {
    ...
    loadData()
    // nextTick(() => {
    //     const gmapObject = drawingManager.value?.$mapObject;
    //     if (gmapObject) {
    //       google.maps.event.addListener(gmapObject, "polygoncomplete", onPolygonComplete);
    //     }
    //   });
    // if (navigator.geolocation) {
    //   setInterval(() => {
    ...
    //     {
    //       enableHighAccuracy: true,
    //       timeout: 5000,   // Tăng thời gian timeout lên 10 giây
    //       maximumAge: 0     // Luôn cập nhật vị trí mới nhất
    //     }// Bật độ chính xác cao
    //   );
    //   }, 5000);
